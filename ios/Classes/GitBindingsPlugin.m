...
void gj_log(const char *message) { NSLog(@"GitJournalLib: %s", message); }
...
- (void)handleMethodCall:(FlutterMethodCall *)call
  ...
  NSArray *sshPublicComponents =
      [NSArray arrayWithObjects:filesDir, @"ssh", @"id_rsa.pub", nil];
  NSString *sshPublicKeyString =
      [NSString pathWithComponents:sshPublicComponents];

  NSArray *sshPrivateComponents =
      [NSArray arrayWithObjects:filesDir, @"ssh", @"id_rsa", nil];
  NSString *sshPrivateKeyString =
      [NSString pathWithComponents:sshPrivateComponents];
  ...
  if ([@"gitClone" isEqualToString:method]) {
    ...
    if (authorName == nil || [authorName length] == 0) {
      result([FlutterError errorWithCode:@"InvalidParams"
                                 message:@"Invalid authorName"
                                 details:nil]);
      return;
    }
    if (authorEmail == nil || [authorEmail length] == 0) {
      result([FlutterError errorWithCode:@"InvalidParams"
                                 message:@"Invalid authorEmail"
                                 details:nil]);
      return;
    }
    ...
    int err =
        gj_git_commit([folderPath UTF8String], [authorName UTF8String],
    ...
    if (content == nil || [content length] == 0) {
      result([FlutterError errorWithCode:@"FAILED"
                                 message:@"PublicKey File not found"
                                 details:nil]);
      return;
    }
    ...
    [publicKey writeToFile:sshPublicKeyString
                atomically:YES
                  encoding:NSUTF8StringEncoding
                     error:&error];
    ...
    [privateKey writeToFile:sshPrivateKeyString
                 atomically:YES
                   encoding:NSUTF8StringEncoding
                      error:&error];
...
- (void)handleMethodCallAsync:(FlutterMethodCall *)call
  ...
  if ([@"gitClone" isEqualToString:method]) {
    ...
    int err = gj_git_pull([folderPath UTF8String], [authorName UTF8String],
    ...
    int err = gj_generate_ssh_keys(sshPrivateKeyPath, sshPublicKeyPath,
                                   [comment UTF8String]);
    if (handleError(result, err)) {
      return;
    }
